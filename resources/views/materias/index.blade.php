...
<div class="container-fluid p-0">
    ...
    <div class="col-12 bg-primario rounded-top mb-3">
        <div class="col-12 py-4">
            <h1 class="text-white fw-bold text-uppercase text-center fs-4 mb-2">Control de información de materias</h1>
            <p class="text-white text-center fs-6 fw-light">Información de las materias</p>
        </div>
    </div>
    <!-- Container Slogan -->
    ...
    <div class="row w-100 h-100 m-0 p-0">
        ...
        <!-- Header Section -->
        <form action="{{ route('materias.index') }}" method="POST" class="row w-100 p-0 mx-auto align-items-center" id="form-number-items">
            ...
            <div class="col-12 col-md-4">
                <a href="{{ route('materias.create') }}" class="btn btn-primario text-white btn-sm d-block mx-auto w-75">Crear Nuevo</a>
            </div>
        </form>
        <!-- Header Section -->
                ...
                <table class="table table-primaria d-none d-lg-table text-center align-center">
                    ...
                    <tbody id="">
                        ...
                        <tr class="d-none d-lg-table-row col-12">
                            <td class="col-3">{{ $item->name1}}</td>
                            <td class="col-3">{{ $item->docente.' '.$item->apellido_paterno.' '.$item->apellido_materno}} </td>
                            <td class="col-1">{{ $item->gruposN}} </td>
                            <td class="col-3">
                                <span
                                    @class([
                                        'badge bg-success' =>  $item->activo1,
                                        'badge bg-danger'  => !$item->activo1
                                    ])
                                >
                                    {{( $item->activo1 ) ? 'Activo' : 'Inactivo' }}
                                </span>
                            </td>
                              ...
                              {{--  start botón de editar --}}
                                <div class="row justify-content-evenly w-100">
                                    <div class="col text-center px-0">
                                        <a href="{{ route('materias.edit', $item) }}" class="btn btn-sm btn-primario text-white w-20" >
                                    ...
                                    <div class="col text-center px-0">
                                        <form action="{{ route('materias.toggle.status', $item) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <button type="submit" @if($item->activo1)
                                                value="Desactivar"
                                                @else
                                                value="Activar"
                                                @endif
                                                @class([
                                                'btn-secundario text-white w-20 btn btn-sm' => $item->activo1,
                                                'btn-success w-20 btn btn-sm' => !$item->activo1
                                                ])
                                    ...
                                    <div class="col text-center px-0">
                                        @if(!$item->activo1)
                                        <form action="{{ route('materias.destroy', $item) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="card shadow-lg my-3 py-3 mx-auto d-lg-none">
            <div class="card-title text-center fw-bold">Grupo: <i>{{ $item->gruposN}}</i></div>
            <div class="card-body text-center">
                <p><b>Materia: </b>{{ $item->name1}}</p>
                <p><b>Docente: </b>{{ $item->docente.' '.$item->apellido_paterno.' '.$item->apellido_materno}}</p>
                @if($item->activo)
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col text-center">
                        <a href="{{ route('estudiantes.edit', $item) }}" class="btn btn-sm btn-primario text-white">Editar</a>
                    </div>
                    <div class="col text-center">
                        <form action="{{ route('estudiantes.toggle.status', $item) }}" method="POST">
                            @csrf
                            @method('PUT')
                    ...
                    @if(!$item->activo)
                    <div class="col text-center">
                        <form action="{{ route('estudiantes.destroy', $item) }}" method="POST">
                            @csrf
                            @method('DELETE')
