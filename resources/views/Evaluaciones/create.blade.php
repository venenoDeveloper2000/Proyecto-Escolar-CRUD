...
<div class="container-fluid p-0">
    ...
    <div class="container-fluid">
        <div class="row justify-content-center">
            ...
            <!-- Form -->
            <form action="{{ route('Evaluaciones.store') }}" class="form" method="POST">
                @csrf
                @method('POST')
                        ...
                        <div class="col-12 col-lg-10">
                            ...
                            <div class="form-floating my-2">
                                <select name="idmat" {{-- <-- Nombre del Campo --}} id="role_select" class="form-select form-control" autocomplete="off"
                                    value="{{ old( 'idmat' ) }}" placeholder="Seleccione la materia" required>
                                    <option value="" selected disabled>Seleccione la materia</option>
                                    @foreach($materias as $m)
                                    <option value="{{$m->idmat_carr}}">{{$m->nombre}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-floating my-2">
                                <select name="idtu" {{-- <-- Nombre del Campo --}} id="role_select" class="form-select form-control" autocomplete="off"
                                    ...
                                    <option value="{{$p->idtu}}">{{$p->nombre.' '.$p->apellido_paterno.' '.$p->apellido_materno}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-floating my-2">
                                <select name="idc" {{-- <-- Nombre del Campo --}} id="role_select" class="form-select form-control" autocomplete="off"
                                    ...
                                    <option value="{{$ce->idc}}">{{$ce->nombre}}</option>
                                    @endforeach
                                </select>
                                <label for="input-text">Seleccione el cuatrimestre</label>
                ...
                <div class="row justify-content-center">
                    <div class="col-12 col-lg-3">
                        <button type="submit" class="btn btn-md d-block w-100 text-white btn-primario">Agregar</button>
                    </div>
                </div>
