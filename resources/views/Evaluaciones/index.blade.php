...
<div class="container-fluid p-0">
    ...
    <div class="col-12 bg-primario rounded-top mb-3">
        <div class="col-12 py-4">
            <h1 class="text-white fw-bold text-uppercase text-center fs-4 mb-2">Evaluaciones</h1>
    ...
    <div class="row w-100 h-100 m-0 p-0">
        ...
        <!-- Header Section -->
        <form action="{{ route('Evaluaciones.index') }}" method="POST" class="row w-100 p-0 mx-auto align-items-center" id="form-number-items">
            ...
            <div class="col-12 col-md-4">
                <a href="{{ route('Evaluaciones.create') }}" class="btn btn-primario text-white btn-sm d-block mx-auto w-75">Asignar evaluación</a>
            </div>
        </form>
        <!-- Header Section -->
        ...
        <div class="row">
            <div class="col-12">
                <table class="table table-primaria d-none d-lg-table text-center align-center">
                    <thead class="table-head fw-bold">  <!-- head de la tabla START-->
                        <th>Nombre Completo</th>
                        <th>Materia</th>
                        <th>Calificación</th>
                        <th>Estado de evaluación</th>
                        <th>&nbsp;</th>
                    </thead>    <!-- head de la tabla END-->
                    <tbody id="">
                        @forelse($items as $item)
                        <tr class="d-none d-lg-table-row col-12">
                            <td class="col-4">{{ $item->nombre_us." ".$item->ap_paterno." ".$item->ap_materno}}</td>
                            <td class="col-1">{{ $item->nombre_ma}}</td>
                            <td class="col-1">{{ $item->calificacion}}</td>
                            <td class="col-3">
                                <span
                                    @class([
                                ...
                                >
                                    {{( $item->activo ) ? 'Asignado' : 'No asignado'}}
                                </span>
                            </td>
                              ...
                              {{--  start botón de editar --}}
                                <div class="row justify-content-evenly w-100">
                                    <div class="col text-center px-0">
                                        <a href="{{ route('Evaluaciones.edit', $item) }}" class="btn btn-sm btn-primario text-white w-20" >
                                    ...
                                    <div class="col text-center px-0">
                                        <form action="{{ route('Evaluaciones.toggle.status', $item) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                    ...
                                    <div class="col text-center px-0">
                                        @if(!$item->activo)
                                        <form action="{{ route('Evaluaciones.destroy', $item) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
