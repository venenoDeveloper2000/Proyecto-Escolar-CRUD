            ...
            @if (isset($idce))
                ...
                @foreach ($listas as $lista)
                    <div class="card shadow-lg my-3 py-3 mx-auto d-lg-none">
                        <div class="card-title text-center fw-bold">Carrera: {{ $lista->nombreCarreras}}</div>
                        <div class="card-body text-center">
                            <p> Alumnos: {{ $lista->numeroGrupos }} </p>
                        </div>
                        <div class="card-body text-center">
                            Grupos: {{ $lista->numeroDocentes }}
                        </div>
                        <div class="card-body text-center">
                            <a href="{{ route('detalle_carreras', ['id' => $lista->iddeLaC, 'idce' => $idce]) }}"
                                class="btn btn-primario text-white btn-sm d-block mx-auto w-75">Detalle</a>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $("#ciclos").change(function() {
                ...
                if (valciclo != 0) {
                    $("#resultado1").load("{{ route('contenido') }}?idce=" + valciclo).serialize();
                    //$("#conte").hide();
                }
            });
        });
