...
<div class="container-fluid p-0">
    ...
    <div class="col-12 bg-primario rounded-top mb-3">
        <div class="col-12 py-4">
            <h1 class="text-white fw-bold text-uppercase text-center fs-4 mb-2">CRUD de Libros</h1>
    ...
    <div class="row w-100 h-100 m-0 p-0">
        ...
        <!-- Header Section -->
        <form action="{{ route('libro.index') }}" method="POST" class="row w-100 p-0 mx-auto align-items-center" id="form-number-items">
            <div class="col-12 col-md-4 text-center my-2">
                @csrf
                @method('GET')
                <select name="paginate" id="select-number-items">
                    <option value="10" @if($consulta->count() == '10') selected @endif >10</option>
                    <option value="50" @if($consulta->count() == '50') selected @endif >50</option>
                    <option value="100" @if($consulta->count() == '100') selected @endif >100</option>
                    <option value="250" @if($consulta->count() == '250') selected @endif >250</option>
                </select>
            </div>
            ...
            <div class="col-12 col-md-4">
                <a href="{{ route('libro.create') }}" class="btn btn-primario text-white btn-sm d-block mx-auto w-75">Crear Nuevo</a>
            </div>
        </form>
        <!-- Header Section -->
                              ...
                              {{--  start botón de editar --}}
                                <div class="row justify-content-evenly w-100">
                                    <div class="col text-center px-0">
                                        <a href="{{ route('libro.edit', $item) }}" class="btn btn-sm btn-primario text-white w-20" >
                                    ...
                                    <div class="col text-center px-0">
                                        <form action="{{ route('libro.toggle.status', $item) }}" method="POST">
                                            @csrf
                                            @method('PUT')
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col text-center">
                        <a href="{{ route('libro.edit', $item) }}" class="btn btn-sm btn-primario text-white">Editar</a>
                    </div>
                    <div class="col text-center">
                        <form action="{{ route('libro.toggle.status', $item) }}" method="POST">
                            @csrf
    ...
    <div class="row my-3 w-100 h-100 p-0">
        <div class="col-12 col-md-3">
            <p class="text-secondary text-center fst-italic">Mostrando {{ $consulta->count() }} resultados de {{ $consulta->total() }}</p>
        </div>
